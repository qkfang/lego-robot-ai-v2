...
export function ServiceButton(props: ButtonProps) {
  ...
  const serial: any = useRef(undefined);

  //let serial: WebSerial | undefined = undefined;
...
sound.beep(500)`
  */
  ...
  const buttonState: ButtonState = new ButtonState("Disconnected", "error", "Not Connected. Connect USB with Lego Hub.");
  ...
  useEffect(() => {
    ...
    const pyreplProxy: any = new Proxy(targetObj, {
      set: function (target: PyreplProxyObject, key: string, value: any) {
        if (key === "write") {
          if (typeof (value) == 'string') {
            serial.current.rawWriteToPort(WebSerial.CONTROL_E)
            const delayInterval = 8 // 8ms per line
            for (let i = 0; i < value.split("\n").length; i++) {
              setTimeout(() => {
                serial.current.rawWriteToPort(value.split("\n")[i] + "\r")
              }, i * delayInterval);
            }
            setTimeout(() => {
              serial.current.rawWriteToPort(WebSerial.CONTROL_D)
            }, delayInterval * value.split("\n").length + 1);
          }
          else {
            serial.current.writeToPort([WebSerial.CONTROL_E, ...(value.split("\n")), WebSerial.CONTROL_D])
          }
        ...
        // Needs to be a function
          ...
          if (typeof value === 'function')
            serial.current.executeAfterInit = value;
          else
          ...
          serial.current.rawWriteToPort(value);
        }
        else if (key === "consoleStream") {
          serial.current.onRead = value;
        }
        return true;
      },
      ...
      get: function (target: PyreplProxyObject, key: string) {
        if (key === "read") {
          target["read"] = serial.current.getOutput();
          return target["read"];
        }
        else if (key === "isActive") {
          return serial.current.isActive;
        }
        else if (key === "clearConsole") {
          serial.current.clearConsole();
        }
        else if (key === "stop") {
          serial.current.rawWriteToPort(WebSerial.CONTROL_C)
        }
        else if (key === "reboot") {
          serial.current.rawWriteToPort(WebSerial.CONTROL_D)
        }
