...
def load_image_filenames() -> list[str]:
    ...
    """
    with open(dataset_filepath, "r", encoding="utf-8-sig") as csv_file:
        ...
        # print(test[1])
        image_filenames = [row['image_file'] for row in csv_reader]
...
def download(image_names: list[str]) -> None:
    for image_url in image_names:
        img_data = requests.get(image_url.replace('PT','PN').replace('.t1','')).content
        img_file = os.path.join(images_folder, image_url.replace('https://img.bricklink.com/ItemImage/PT/','').replace('/','-').replace('.t1',''))
        print(img_file)
